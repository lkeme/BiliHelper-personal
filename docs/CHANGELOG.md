...
- 登录模块新增一次性网页登录助手，支持在浏览器中处理行为验证码、短信验证码与风控短信验证码，Docker 用户无需再进入容器手动输入。
- 登录助手新增统一人工流程状态接口与前端页面，可在当前运行内完成人工登录步骤并自动清理一次性 flow。

### Changed

- 登录助手页面完成现代化重构，收敛卡片层级、间距与结果区布局，提升整体可读性且不影响原有交互功能。
- Docker 示例配置默认启用登录助手服务，文档同步统一本地访问地址为 `127.0.0.1`，降低 Docker 与本地用户的使用门槛。

### Fixed

- 修复 Docker 场景下登录时需要进入容器输入验证码的问题，将人工输入入口统一迁移到容器外浏览器页面。
- 优化短信验证码输入校验，前端现要求 6 位纯数字，减少无效验证码提交。

### Remarks

- 发布版本提升至 `3.0.4.260425`，同步更新 `README`、`docs/CHANGELOG.md` 与 `resources/version.json`。
