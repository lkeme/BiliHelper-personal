...
class MainSiteArchiveService
{
    ...
    public function followUpArchives(int $num): array
    {
        ...
        if (($response['code'] ?? 0) !== 0) {
            Log::warning("主站任务: 获取关注动态失败 {$response['code']} -> {$response['message']}");
        } else {
            $archives = $this->extractDynamicArchives($this->extractFollowUpItems($response['data'] ?? null), $num);
            Log::info('主站任务: 命中关注动态稿件 ' . count($archives));
        }

        if (($currentNum = count($archives)) < $num) {
            $fallback = $this->topArchives($num - $currentNum);
            if ($fallback !== []) {
                Log::warning('主站任务: 关注动态稿件不足，将自动补全随机稿件。');
                Log::info('主站任务: 自动补全随机稿件 ' . count($fallback));
            }
     ...
     * @return array<string, mixed>
     */
    public function takeLastArchive(array $archives): array
    {
    ...
    {
        return ApiVideo::dynamicRegion($ps);
    }
    ...
    {
        return ApiDynamicSvr::followUpDynamic();
    }
