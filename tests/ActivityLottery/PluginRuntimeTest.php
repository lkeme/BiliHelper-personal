...
Assert::same(1, (int)($businessExecuteLog['context']['node_position'] ?? 0), '业务执行日志应包含当前节点序号。');
Assert::same(1, (int)($businessExecuteLog['context']['node_total'] ?? 0), '业务执行日志应包含节点总数。');
...
Assert::same('等待继续', (string)($followWaitingSummaryLog['context']['node_status_label'] ?? ''), '关注摘要日志应包含等待状态标签。');
Assert::true(str_contains((string)$followWaitingSummaryLog['message'], '流程进度 1/1'), '关注摘要日志应包含流程进度。');
Assert::true(str_contains((string)$followWaitingSummaryLog['message'], '当前阶段：关注任务'), '关注摘要日志应包含当前阶段。');
Assert::true(str_contains((string)$followWaitingSummaryLog['message'], '状态：等待继续'), '关注摘要日志应包含等待状态。');
...
Assert::same('等待继续', (string)($videoWaitingSummaryLog['context']['node_status_label'] ?? ''), '视频摘要日志应包含等待状态标签。');
Assert::true(str_contains((string)$videoWaitingSummaryLog['message'], '流程进度 1/1'), '视频摘要日志应包含流程进度。');
Assert::true(str_contains((string)$videoWaitingSummaryLog['message'], '当前阶段：观看视频'), '视频摘要日志应包含当前阶段。');
Assert::true(str_contains((string)$videoWaitingSummaryLog['message'], '状态：等待继续'), '视频摘要日志应包含等待状态。');
...
Assert::same('等待继续', (string)($liveWaitingSummaryLog['context']['node_status_label'] ?? ''), '直播摘要日志应包含等待状态标签。');
Assert::true(str_contains((string)$liveWaitingSummaryLog['message'], '流程进度 1/1'), '直播摘要日志应包含流程进度。');
...
Assert::same('已完成', (string)($drawRefreshSummaryLog['context']['node_status_label'] ?? ''), '刷新抽奖次数摘要日志应包含完成状态标签。');
Assert::true(str_contains((string)$drawRefreshSummaryLog['message'], '流程进度 1/1'), '刷新抽奖次数摘要日志应包含流程进度。');
Assert::true(str_contains((string)$drawRefreshSummaryLog['message'], '当前阶段：抽奖阶段'), '刷新抽奖次数摘要日志应包含阶段。');
Assert::true(str_contains((string)$drawRefreshSummaryLog['message'], '状态：已完成'), '刷新抽奖次数摘要日志应包含完成状态。');
...
Assert::same('等待继续', (string)($drawExecuteSummaryLog['context']['node_status_label'] ?? ''), '执行抽奖摘要日志应包含等待状态标签。');
Assert::true(str_contains((string)$drawExecuteSummaryLog['message'], '流程进度 1/1'), '执行抽奖摘要日志应包含流程进度。');
Assert::true(str_contains((string)$drawExecuteSummaryLog['message'], '当前阶段：抽奖阶段'), '执行抽奖摘要日志应包含阶段。');
Assert::true(str_contains((string)$drawExecuteSummaryLog['message'], '状态：等待继续'), '执行抽奖摘要日志应包含等待状态。');
...
Assert::true(str_contains((string)$drawSummaryFlowLog['message'], '状态：已完成'), '抽奖汇总摘要日志应包含完成状态。');
